...
The program starts with a list of all the possible final answers that a given Wordle can have. It types in a starting word (chosen by the user), then uses the results to eliminate words that aren’t compatible with those results. The next word the program chooses is the alphabetical first word out of the remaining ones.
